...
const JobOpeningDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const calendar_24Template3 = (rowData, { rowIndex }) => <p>{new Date(rowData.createdAt).toLocaleDateString()}</p>
...
<Column field="createdAt" header="CreatedAt" body={calendar_24Template3} filter={selectedFilterFields.includes("createdAt")} hidden={selectedHideFields?.includes("createdAt")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload JobOpening Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="jobOpening"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search JobOpening" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default JobOpeningDataTable;