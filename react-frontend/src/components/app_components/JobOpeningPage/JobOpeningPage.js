...
import JobOpeningEditDialogComponent from "./JobOpeningEditDialogComponent";
import JobOpeningCreateDialogComponent from "./JobOpeningCreateDialogComponent";
import JobOpeningFakerDialogComponent from "./JobOpeningFakerDialogComponent";
import JobOpeningSeederDialogComponent from "./JobOpeningSeederDialogComponent";
...
const JobOpeningPage = (props) => {
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("jobOpening");
    ...
    useEffect(() => {
        ...
        client
            .service("jobOpening")
            .find({ query: { $limit: 10000 , id : urlParams.singleApplicationId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("jobOpening").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "JobOpening", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("jobOpening").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="jobOpening-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="jobOpening-datatable">
                    <JobOpeningDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <JobOpeningEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <JobOpeningCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <JobOpeningFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <JobOpeningSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(JobOpeningPage);
