...
import jobOpeningFakerFactory from "./jobOpeningFakerFactory";
...
const JobOpeningFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jobOpeningFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jobOpening").create(elem));
...
export default connect(mapState, mapDispatch)(JobOpeningFakerDialogComponent);
