...
import applicationFakerFactory from "./applicationFakerFactory";
...
const ApplicationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = applicationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("application").create(elem));
...
export default connect(mapState, mapDispatch)(ApplicationFakerDialogComponent);
