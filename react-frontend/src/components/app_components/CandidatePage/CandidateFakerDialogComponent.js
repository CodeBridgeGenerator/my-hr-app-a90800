...
import candidateFakerFactory from "./candidateFakerFactory";
...
const CandidateFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = candidateFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("candidate").create(elem));
...
export default connect(mapState, mapDispatch)(CandidateFakerDialogComponent);
