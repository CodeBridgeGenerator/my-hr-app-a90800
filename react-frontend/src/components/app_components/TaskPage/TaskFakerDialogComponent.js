...
import taskFakerFactory from "./taskFakerFactory";
...
const TaskFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = taskFakerFactory(props.user,fakerCount,assignedToIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("task").create(elem));
...
export default connect(mapState, mapDispatch)(TaskFakerDialogComponent);
